...
public class UserUnit extends BaseJunit4 {
	...
	public void unit() {
		try {
			...
			// pb.setResults(secretRobotService.selectSecretRobotList(pb));
			// System.err.println(pb);
			// String[] users = { "3eeb014c2d4341d78a95c806e716c4ba" };
			// Map<String, String> map = new HashMap<String, String>();
			// map.put("action", "action!!!!!!!!!!!!!叹气红");
			// SingleChatMsg singleChatMsg = new
			// 33c50d388646485ba1d98d1a3aafd002
			// SingleChatMsg(EasemobSingleChatMsgTypeEnum.CMD, "msg");
			// (String targetType,String[] targets, String from, Map<String,
			// String> ext, String msg)
			// CmdMessageBody msgBody2 = new CmdMessageBody(
			// EasemobSingleChatSendTypeEnum.SNED_TO_USERS.getValue(), users,
			// "admin", map,
			// singleChatMsg.getMsg());
	...
	// int moyouId = 0;
	// String moyouIdString =
	// redisTemplate_KVString.opsForList().rightPop(CONSTANT.MOYOUID_LIST);
	// if (StringUtils.isEmpty(moyouIdString)) {
	// // moyouId取值区间
	// GenerateNumberRecord numberRecord =
	// numberRecordMapper.queryRangeOfMoyouId();
	// int startNum = numberRecord.getStartNumber(), endNum =
	// numberRecord.getEndNumber();
	// int surplus = endNum - startNum;// 区间值
	// if (numberRecord.getRegisterCount() >= surplus) {// 用户注册超过区间，需要新生成一条区间值记录
	// startNum = endNum;
	// endNum = startNum + surplus;
	// GenerateNumberRecord newGenerateNumberRecord = new
	// GenerateNumberRecord();
	...
	// moyouIdList);
	// moyouId = Integer.parseInt(redisTemplate_KVString.opsForList().rightPop(
	// CONSTANT.MOYOUID_LIST));
	// } else {
