...
//        String deviceid = request.getHeader("deviceid");
        ...
        // token验证
        int status = 200;
//        logger.info("tokentokentokentokentokentoken=="+token);
//        byte[] tokenValueByte = redisTemplate.opsForValue().get(CONSTANT.H5_USER_TOKEN_PREFIX + token);// 如果访问接口的时间戳与当前时间差值大于1个小时，则返回接口访问超时
        String tokenValue = redisTemplate_KVString.opsForValue().get(CONSTANT.H5_USER_TOKEN_PREFIX + token);// 如果访问接口的时间戳与当前时间差值大于1个小时，则返回接口访问超时
//         logger.info("tokentokentokentokentokentoken=="+tokenValue);
