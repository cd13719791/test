...
public class ClientContext {
	...
	public static final String PROPERTIES_PATH = "easemob.properties";
	/*
	 * Implementation List
	 */
...
//			log.warn("Context has been initialized already, skipped!");
	...
	private void initFromPropertiesFile() {
		...
		try {
			InputStream inputStream = ClientContext.class.getClassLoader().getResourceAsStream(PROPERTIES_PATH);
			p.load(inputStream);
		} catch (IOException e) {
			log.error(MessageTemplate.print(MessageTemplate.FILE_ACCESS_MSG, new String[]{PROPERTIES_PATH}));
		...
		if( StringUtils.isBlank(protocal) || StringUtils.isBlank(host) || StringUtils.isBlank(org) || StringUtils.isBlank(app) || StringUtils.isBlank(clientId) || StringUtils.isBlank(clientSecret) || StringUtils.isBlank(impLib) ) {
			log.error(MessageTemplate.print(MessageTemplate.INVAILID_PROPERTIES_MSG, new String[]{PROPERTIES_PATH}));
			return; // Context not initialized
		}
