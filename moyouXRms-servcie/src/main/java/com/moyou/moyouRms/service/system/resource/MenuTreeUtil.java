    ...
    public static String buildTreeHtml(List<Resources> menusDB){  
        ...
        for (int i=0;i<menusDB.size();i++) { 
            ...
            if ("0".equals(node.getParentId())) { 
            	...
            	html.append("\r\n<a ");
            	html.append(" href='#maincontent' style='cursor:pointer;'  onclick=\"openMenu('"+node.getType()+"','menu"+node.getId()+"','menu0','"+node.getName()+"','"+(StringUtil.isNotBlank(node.getResUrl())?node.getResUrl().trim():"noset")+"')\" ");       		    	
            	if(childrenHas)html.append(" target='mainFrame' class='dropdown-toggle' "); 
            	html.append(" >"); 
            	if(StringUtil.isNotEmpty(node.getIcon()))html.append("\r\n<i class= "+ node.getIcon()+" ></i>"); 
    ...
    private static void build(List<Resources> menusDB,Resources node,StringBuffer html){  
        ...
        if (!children.isEmpty()) {  
            ...
            for (Resources child : children) {  
            	...
            	html.append("\r\n<a ");
        		html.append(" href='#maincontent' style='cursor:pointer;' onclick=\"openMenu('"+child.getType()+"','menu"+child.getId()+"','menu"+child.getParentId()+"','"+child.getName()+"','"+(StringUtil.isNotBlank(child.getResUrl())?child.getResUrl().trim():"noset")+"')\" ");
            	if(childrenHas)html.append(" target='mainFrame' class='dropdown-toggle' "); 
            	html.append(" >"); 
            	if(!"0".equals(child.getParentId()))html.append("\r\n<i class='icon-double-angle-right' ></i>"); 
            	html.append("\r\n<span class='menu-text'>");
            	if(StringUtil.isNotEmpty(child.getIcon())){
