package com.moyou.moyouRms.manager.schedule;

import java.util.Date;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import com.moyou.moyouRms.model.po.task.log.TaskLog;
import com.moyou.moyouRms.service.task.TaskLogService;
import com.moyou.moyouRms.spring.SpringBeanUtils;
public class TaskLogManager {
	/** 日志对象 */
    private static final Logger LOG = LoggerFactory.getLogger(TaskLogManager.class);
	private static TaskLogService service;
	
	public final static int NORMAL=1;
	
	public final static int EXCEPTION=2;
	
	public static TaskLogService getTaskLogService(){
		if(service==null){
			service = (TaskLogService)SpringBeanUtils.getFirstBeanOfType(TaskLogService.class);
		}
		return service;
	}
	
	public static void saveTaskLog(String name,String className,int type,String dec){
		TaskLog o =new TaskLog(name,className,type,dec,new Date());
		saveTaskLog(o);
	}
	
	public static void saveTaskLog(TaskLog o){
		try {
			getTaskLogService().insert(o);
		} catch (Exception e) {
			LOG.error("保存任务日志失败",e);
		}
	}
	
}
